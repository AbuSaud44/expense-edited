...
def ensure_id(df: pd.DataFrame) -> pd.DataFrame:
    # Guarantee an integer 'id' column for edit/delete operations
    if "id" not in df.columns:
        ...
        # Reassign if any missing
        if df["id"].isna().any():
            # Keep order, re-id
            df = df.reset_index(drop=True)
            df["id"] = range(1, len(df)+1)
    df["id"] = df["id"].astype(int) if not df.empty else df.get("id", pd.Series(dtype="int"))
    return df
...
def compute_kpis(df: pd.DataFrame):
    ...
    return total, avg_day, max_day, int(len(df))
    ...
    with st.form("add_expense", clear_on_submit=True):
        ...
        if submitted:
            ...
            new_row = {
                "id": next_id(df),
                "date": d,
                "category": cat if cat.strip() else "Uncategorized",
                "amount": amt,
                "notes": note
            }
...
if not df_filtered.empty:
    ...
    options = []
    for _, r in df_filtered.sort_values("date").iterrows():
        label = f'#{int(r["id"])} | {r["date"]} | {r["category"]} | {float(r["amount"]):.2f}'
        options.append((int(r["id"]), label))
    selected_label = st.selectbox("Select a record", options, format_func=lambda x: x[1] if isinstance(x, tuple) else x)
    selected_id = selected_label[0] if isinstance(selected_label, tuple) else selected_label

    # Get live snapshot
    df_all_live = load_data()
    row = df_all_live[df_all_live["id"] == selected_id]
    if not row.empty:
        r = row.iloc[0]
        with st.form("edit_form"):
            ed_date = st.date_input("Date (edit)", value=r["date"])
            ed_cat = st.text_input("Category (edit)", value=r["category"])
            ed_amt = st.number_input("Amount (edit)", min_value=0.0, step=1.0, format="%.2f", value=float(r["amount"]))
            ed_note = st.text_input("Notes (edit)", value=r["notes"])

            col1, col2 = st.columns(2)
            save_btn = col1.form_submit_button("💾 Save changes")
            del_btn = col2.form_submit_button("🗑️ Delete record")
    ...
    st.info("No rows to edit/delete for current filters.")
